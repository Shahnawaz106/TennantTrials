...
function ApartmentCard({ apt }) {
  return (
    <Link to={`/apartment/${apt.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
...
export default function Dashboard() {
  ...
  // useEffect: update browser tab title whenever filtered count changes
  useEffect(() => {
    document.title = `TenantTrails — ${filtered.length} apartments`
    return () => { document.title = 'TenantTrails' }
  }, [filtered.length])
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
      ...
      <div style={styles.container}>
        ...
        {/* Conditional rendering: toggle shows/hides filters */}
        <button onClick={() => setShowFilters(!showFilters)} style={styles.filterToggleBtn}>
...
const styles = {
  ...
  card: { background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden', cursor: 'pointer', transition: 'box-shadow 0.2s' },
